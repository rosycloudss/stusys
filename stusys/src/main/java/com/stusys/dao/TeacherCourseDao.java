...
public class TeacherCourseDao {
	...
	public int update(TeacherCourse teaCourse) {
		int affectColums = 0;
		String sql = "UPDATE TB_TEACHER_COURSE SET COURSE_NO=?,TEACHER_NO=?,SEMESTER=?,TEACH_ADDRESS_TIME=?,STUDENT_NUM=?,SELECT_NUM=? WHERE TC_NO=?";
		try {
			conn = DBUtil.getConnection();
			prestat = conn.prepareStatement(sql);
			prestat.setLong(1, teaCourse.getCourse().getCourseNo());
	...
	public List<TeacherCourse> select(TeacherCourse teaCourse, Page page) {
		...
		if (teaCourse != null) {
			...
			if (teaCourse.getStudentNum() != 0) {
				sql.append(" AND STUDENT_NUM=?");
			}
			if (teaCourse.getSelectNum() != 0) {
